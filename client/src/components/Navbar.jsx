...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16  lg:px-24 mt-0 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
    ...
    <div className="hidden sm:flex items-center gap-8">
        ...
        <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
            <img className="h-6 w-6 opacity-80" src={assets.nav_cart_icon} alt="cart"/>
    ...
    <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-50`}>
        ...
        {user && <NavLink className="block" onClick={()=> setOpen(false)} >My orders</NavLink>}
        <NavLink to='/products'   onClick={()=> setOpen(false)} className="block">Contact</NavLink>
